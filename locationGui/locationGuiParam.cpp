...
locationGuiParam::locationGuiParam()
{
	iniData();
}
...
void locationGuiParam::readWriteParam(bool r, const QString& dirPath, int level)
{
	SETTING_INIT(dirPath + "/locationGui.ini");

	if (r&&level != ProductLevel)
		return;

	RW_VALUE(r, edgeSearchOrder);
	...
	RW_VALUE1(r, standOffCorr[0], 0);
	RW_VALUE1(r, standOffCorr[1], 1);
	RW_VALUE1(r, widthCorr[0], 0);
	RW_VALUE1(r, widthCorr[1], 1);
	RW_VALUE1(r, InterCorr[0], 0);
	RW_VALUE1(r, InterCorr[1], 1);
	...
	for (int i = 0;i < 4;i++)
	{
		...
		RW_VALUE1(r, edgesearchWidth[i], i);
		RW_VALUE1(r, pinSearchWidth[i], i);
		RW_VALUE1(r, clipfactor[i], i);
		RW_VALUE1(r, IsUseStand[i], i);
	}
	...
	RW_VALUE(r, LTnum);
	RW_VALUE(r, RBnum);
	RW_VALUE(r, ChildTablenum);
}

void locationGuiParam::iniData()
{
	edgeSearchOrder = 0;

	IsEndPoint = 0;      //是否检测端点0关闭1启用
	upDwOrLfRt = 1;
	maxAngle = 10;
	UpDetect = DwDetect = LfDetect = RtDetect = 1;
	standOffCorr[0] = 0;    //站立度修正
	standOffCorr[1] = 0;

	widthCorr[0] = widthCorr[1] = 0;
	InterCorr[0] = InterCorr[1] = 0;

	pinlength = 0.65;
	for (int i = 0;i < 4;i++)
	{
		whiteBlackDir[i] = searchDir[i] = 0;
