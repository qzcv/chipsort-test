...
void edgeLocationWdg::connectSlots(bool link)
{
	...
	connectOrNot(link, ui->cob_showRegion, SIGNAL(currentIndexChanged(int)), SLOT(comboBox_indexChanged(int)));
}
...
void edgeLocationWdg::setUiValue()
{
	...
	ui->cob_showRegion->setCurrentIndex(m_param->IsShowRegionAfterDet);
...
void edgeLocationWdg::on_bt_setOrigin_clicked()
{
	...
	if (QMessageBox::information(this, tr("Hint"), tr("Are you sure to change the current center point to the template center?"),
...
void edgeLocationWdg::radioButton_toggled(bool check)
{
	...
	if (rb == ui->rb_inToOut) {
		...
		if (check)
			str1 = tr(" change from %1 to %2").arg(ui->rb_outToIn->text()).arg(ui->rb_inToOut->text());
		else
			str1 = tr(" change from %1 to %2").arg(ui->rb_inToOut->text()).arg(ui->rb_outToIn->text());
		str2 = ui->tabWidget->tabText(m_nowGrpIndex);
...
void edgeLocationWdg::spinBox_valueChanged(int val)
{
	...
	if (sp == ui->sp_maxAngle) {
		...
		if (sp == ui->sp_extendSearchLine) {
			...
			str0 = ui->cb_fixWidth->text();
			str1 = tr(" change from %1 to %2").arg(QString::number(m_param->fixWidth[m_nowGrpIndex])).arg(QString::number(val));
...
void edgeLocationWdg::comboBox_indexChanged(int index)
{
	...
	if (cb == ui->cob_showRegion) {
		str1 = tr(" change from %1 to %2")
			.arg(ui->cob_showRegion->itemText(m_param->IsShowRegionAfterDet))
			.arg(ui->cob_showRegion->currentText());
...
void edgeLocationWdg::on_bt_selectRoi_toggled(bool check)
{
	int roiNum = 0;
	...
	if (check)
	{
		...
		QString regionName[4] = { tr("up"),tr("right"),tr("down"),tr("left") };
		QStringList title;
		for (size_t i = 0;i < roiNum;++i)
		{
			...
			{
				t = QString("%1_%2").arg(regionName[grpIndex]).arg(i - beforeNum);
...
void edgeLocationWdg::on_bt_HelpRect_toggled(bool check)
{
	if (check) {
