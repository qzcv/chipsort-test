...
int edgeLocationMod::run(const QString &funName)
{
	...
	if (m_param->IsCsp) {
		...
		p_OFFCenter->x = offcx*mk;
		p_OFFCenter->y = offcy*mk;
...
void edgeLocationMod::textResult(ResultText *text, const QString &funName)
{
	...
	if (m_param->IsErrWidthDiff) {
		...
		text->append(QObject::tr("Body Width difference= %1 (%2)").arg(QString::number(errwidthdiff[0].D(), 'f', 3)).arg(QString::number(m_param->errWidthDiff, 'f', 3)));
	}
	if (m_param->IsErrHeight) {
	...
	if (m_param->IsErrHeightDiff) {
		...
		text->append(QObject::tr("Body Height difference= %1 (%2)").arg(QString::number(errheightdiff[0].D(), 'f', 3)).arg(QString::number(m_param->errHeightDiff, 'f', 3)));
	}
	if (m_param->IsErrAngle) {
	...
	if (m_param->IsCsp) {
		...
		text->insertPlainText(QString(" x= %1; y=%2 ")
...
void edgeLocationMod::iniDetector()
{
	...
	for (int i = 0;i < 4;i++) {
		// up right dw left
		m_edgeLine[i]->setPolarAndOrient((danke::fitEdgeLine::Polarity)m_param->whiteBlackDir[i], m_dir[m_param->searchDir[i]][i]);
